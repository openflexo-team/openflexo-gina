...
/**
 * A {@link ModuleView} suitable for {@link FlexoConceptInstance} that have the {@link FMLControlledFIBFlexoConceptInstanceNature}<br>
 * Display a FIB view bound to {@link FlexoConceptInstance} evaluation context.<br>
...
public class FMLControlledFIBFlexoConceptInstanceModuleView extends JPanel implements ModuleView<FlexoConceptInstance> {

	protected static final Logger logger = FlexoLogger
			.getLogger(FMLControlledFIBFlexoConceptInstanceModuleView.class.getPackage().getName());
	...
	public FMLControlledFIBFlexoConceptInstanceModuleView(FlexoConceptInstance representedObject, FlexoController controller,
		...
		this.flexoConceptInstance = representedObject;
		...
		if (getRepresentedObject() != null && component != null) {
			component.bindTo(getRepresentedObject().getFlexoConcept(),
					FMLControlledFIBFlexoConceptNature.getFIBComponentModelSlot(representedObject.getFlexoConcept()));
		}
	...
	public boolean _switchToEditMode() {
		...
		if (editorController == null) {
			...
			editorController = getFIBEditor(false).openFIBComponent(
					...
					flexoConceptInstance, controller.getFlexoFrame());
		}
	...
	private void updateAsEditMode() {
		...
		for (VariableAssignment variableAssignment : fibComponentModelSlot.getAssignments()) {
			try {
